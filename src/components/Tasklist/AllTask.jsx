...
function AllTask({ tasks, colors, getRandomColor,setSelectedTask }) {
    return (
        <>
            {tasks.map((task, index) => (
                <div className={`flex-shrink-0 h-full w-[300px] ${getRandomColor(colors)} rounded-xl`} key={index} onClick={() => setSelectedTask(task.id)} >
